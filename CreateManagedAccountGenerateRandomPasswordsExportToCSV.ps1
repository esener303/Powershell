...
$path = "C:\SharePoint Service Users.csv" 
...
$users = @(
         ...
         @{Username = "SpCacheSuperUser"; Password = random-password;},
         @{Username = "SpCacheSuperReader"; Password = random-password;},
...
New-ADOrganizationalUnit -Name "SharePoint Service Users" -ProtectedFromAccidentalDeletion $false -ManagedBy "SpSetupUsr"
 
# Create each of the users
foreach($user in $users)
{
    New-ADUser $user.Username -AccountPassword (ConvertTo-SecureString -AsPlainText $user.Password -Force) -Enabled $true -Path "OU=SharePoint Service Users,DC=FOUNDATION,DC=local"
}
